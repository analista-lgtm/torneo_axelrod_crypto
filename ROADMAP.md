...
* **Validación multi-horizonte de los pronósticos del tablero — Aprobada ✅:** `src/vol_multihorizon.py` sometió el GJR multi-paso (lo que alimenta las columnas de pronóstico) al protocolo completo en los 36 mercados y 3 horizontes. Resultados (QLIKE selección, menor = mejor): **h=1d: GJR 1.560** vs HIST 1.635 / EWMA 1.581 (gana en 32/36, DM sig. 25) · **h=5d: 0.443** vs 0.520 / 0.467 (32/36, DM sig. 23) · **h=21d: 0.324** vs 0.437 / 0.387 (32/36, DM sig. 19) — la ventaja relativa es *mayor* en el horizonte largo (~26% mejor que la persistencia). En el período sagrado GJR también gana a 1d y 21d y empata con EWMA a 5d. Las tres extrapolaciones del tablero (horizontes largos, mercados nuevos, multi-paso) quedan respaldadas por evidencia directa.
* **Ampliación (versión final del tablero):** universo extendido a **36 mercados en 8 categorías** (10 índices globales incl. FTSE/Hang Seng/Bovespa, renta fija y crédito HY, 6 divisas, 4 metales, 3 energías, 4 agrícolas, 5 criptos, REITs) con **pronósticos GJR-GARCH en vivo** (el ganador de la Fase 2) de la vol esperada a 1 día, 1 semana y 1 mes por mercado — recursión multi-paso hacia la varianza incondicional, EWMA de respaldo — más la **tendencia esperada** (¿sube/baja?) y alertas cuando el pronóstico apunta al alza desde percentiles ya altos. El VRP del S&P ahora usa la vol *prevista* (VIX vs. pronóstico 21d), no la realizada. Tabla del dashboard agrupada por categoría.
...
* **Fase 3 — Integración (Exp. 16): Completada. Veredicto: el statu quo gana.**
  * **Implementación:** `src/vol_target_v3.py` — el mismo portafolio 40/60 (señales, pesos y costos idénticos) con tres termostatos: rodante 63d (statu quo), pronóstico GJR-GARCH walk-forward y EWMA. Muestra común de 3,448 días, bootstrap pareado por bloques para la diferencia de Sharpe.
  * **Resultados:** rodante Sharpe **1.02** | GJR 0.98 (ΔSharpe −0.04, p=0.72) | EWMA 0.94 (−0.08, p=0.94). El GJR sí recortó levemente el drawdown (−17.6% vs −18.9%) pero a costa de retorno.
  * **Lección (valiosa precisamente por negativa): buen pronóstico ≠ mejor cartera.** El vol-targeting se beneficia de estimaciones *suaves de mediano plazo*; un pronóstico a 1 día — aunque gane el torneo QLIKE — es más reactivo y agita el apalancamiento día a día (costo de whipsaw implícito). El horizonte del pronóstico debe casar con el horizonte de la decisión: para targeting con rebalanceo lento, la vol rodante de 63d ya captura casi todo el beneficio (Moreira & Muir usan varianza *mensual*, no GARCH diario). El pronóstico GJR conserva su valor donde su horizonte aplica: monitoreo, alertas y gestión de riesgo de corto plazo (Fases 1-2).
  * **Estado de la línea de volatilidad:** cerrada con saldo claro — la vol es predecible (Fase 2, hecho demostrado), el tablero de monitoreo queda operativo (Fase 1), y la integración al portafolio no supera al estándar simple (Fase 3). El 40/60 con rodante 63d permanece como el constructo oficial del laboratorio.
