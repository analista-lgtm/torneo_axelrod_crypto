...
| 15 | `src/implementation_check.py` | Experimento 15 ⭐: implementabilidad — Sharpe(w) es meseta [10%, 55%]; el w*=40% elegido honestamente **aprueba el sagrado** (+14.7%, Sharpe 1.20, DD −6%); rebalanceo mensual no cuesta nada; crisis alpha medido (peores meses B&H −5.1% → mezcla −3.0%); asimetría positiva | `data/multi_activo/implementacion.json` |
| 16 | `src/vol_monitor.py` | Tablero de Volatilidad (Fase 1): vol realizada multi-horizonte, Yang-Zhang/Parkinson (requiere OHLC, ahora conservado por `data_pipeline`), EWMA, vol-of-vol, percentiles, estructura temporal, correlación media, absorption ratio, VIX/OVX/GVZ y VRP, con alertas | `data/multi_activo/volatilidad.json` |
| 17 | `src/vol_forecast_lab.py` | Fase 2: torneo de pronóstico de varianza a 1 día (QLIKE, walk-forward, DM vs HAR) — **gana GJR-GARCH** (12 de 18 mercados, DM sig. en 9); EWMA subcampeón; HIST-21 no gana en ninguno (la vol ES predecible); LightGBM pierde con claridad. Requiere `arch`, `lightgbm`, `scikit-learn` | `data/multi_activo/vol_modelos.json` |
| 18 | `src/vol_target_v3.py` | Exp. 16 (Fase 3): el 40/60 con tres termostatos de vol-targeting — **el rodante 63d gana** (Sharpe 1.02 vs 0.98 GJR, p=0.72): buen pronóstico ≠ mejor cartera; el horizonte del pronóstico debe casar con el de la decisión. `meta_portfolio_v2.construir` quedó refactorizado en `construir_base` + `vol_target_rodante` | `data/multi_activo/vol_target_v3.json` |
| 19 | `src/vol_multihorizon.py` | Validación multi-horizonte de los pronósticos del tablero: GJR multi-paso vs varas planas en 36 mercados a 1/5/21 días — **aprobada** (gana en 32/36 en los tres horizontes; la ventaja crece con el horizonte: ~26% mejor que persistencia a 21d; sagrado también aprueba). El tablero de 36 mercados con pronósticos queda con sello completo | `data/multi_activo/vol_multihorizonte.json` |
